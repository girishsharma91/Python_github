...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        Label(self, text = "Geometric Standard Deviation").grid(row = 17, column =0, columnspan = 2, sticky = W)
        Label(self, text = " dimensionless ").grid(row = 17, column =3, sticky = W)
        Label(self, text = "Vapor Phase Concentration ").grid(row = 18, column =0, columnspan = 2, sticky = W)
        Label(self, text = " #/cm3 ").grid(row = 18, column =3, sticky = W)
        ...
        def_vg_zero = StringVar(self,value = '0e0')
        self.vg_zero = Entry(self, textvariable = def_vg_zero)
        self.vg_zero.grid(row=16, column =2, sticky =W)
        ...
        def_n1_zero = StringVar(self,value = '0e0')
        self.n1_zero = Entry(self, textvariable = def_n1_zero)
        self.n1_zero.grid(row=18, column =2, sticky =W)
    ...
    def input_param(self):
        ...
        dpg_zero = 1.0e-9*float(self.vg_zero.get()) # nm to m
        sigma_g_zero = float(self.sigma_g_zero.get()) 
        n1_zero = 1.0e6*float(self.n1_zero.get())   # #/cm3 to #/m3
        vg_zero = (22.0/7)/6*(dpg_zero)**(3.0)      
        initial = [num_zero,vg_zero,sigma_g_zero,n1_zero]
...
def coag(sd,rg,flag):
    if flag == 0 or sd <= 1e-20 or rg <= 1e-40:
        ...
        com2fm = 2 * c1 * b2 * ma.exp(3.0/2*lnsig) * ( ma.exp(25.0/8* lnsig) + 2.0*ma.exp(5.0/8*lnsig) + ma.exp(1.0/8*lnsig) )
        com2cn = 2 * c2 * ( 1.0 + ma.exp(lnsig) + b5 * (lam/rg) * ma.exp(-1.0/2 * lnsig) * (1.0 + ma.exp(-2.0*lnsig)))
...
def nucl(S,flag):
    if S < 1.00000000000000000000000000000000000000000001 or flag == 0:
        nuc = [ 0.0 , 0.0 ]
        return nuc
    else:
...
def vectorfield(t , y):
    ...
    dxNUC_N =   x11 / ns * tau
    dxNUC_V =   x11 / ns * kstar * tau
    dxNUC_W =   x11 / ns * kstar * kstar * tau
    ...
    if y[0] > 1e-50 and y[1] > 1e-50:
        ...
        dxCON_V =    coefm1 * y[0] * ( S - 1.0 ) * tau /v1
        dxCON_W =    coefm2 * y[1] * ( S - 1.0 ) * tau /v1
    else:
        dxCON_S =    0.0       
        dxCON_V =    0.0
        dxCON_W =    0.0
    ...
    if y[0] > 1e-50 and y[1] > 1e-50 and y[2] > 1e-50:
        dxCOA_N =    com0 * y[0] ** 2.0 * ns * tau
        dxCOA_W =    com2 * y[1] ** 2.0 * ns * tau
    else:
        dxCOA_N =    0.0
        dxCOA_W =    0.0
...
def main(initial):
    ...
    # DERIVED CONSTANTS
    ns = Ps / (kb*T)
    mu=1.716e-5 * (T/273)**(2.0/3)
    lam = mu/P * ma.sqrt(PI*kb*T/(2*M/Na))
    ...
    n1_zero = initial[3]                                                      # Vapor Concentration (#/m3)  at t=0 
    ...
    r = ode(vectorfield).set_integrator('vode', method='bdf')
    ...
    N_t_zero = num_zero/ns
    V_t_zero = moment(num_zero,v_g_zero,sigma_g_zero,1)/(v1*ns)
    V2_t_zero  = moment(num_zero,v_g_zero,sigma_g_zero,2)/(v1*v1*ns)
    S_t_zero = n1_zero/ns
...
#         i = i+1    
    ...
    plt.semilogy(t*tau/60, N_t*ns*1e-6,color="blue", linewidth = 1.0, linestyle ="-")
    plt.title('Lognormal Size Distribution Parameters')
    plt.subplot(312)
    plt.grid(True)
    plt.ylabel('Average diameter dp (nm)')
    plt.plot(t*tau/60, dp_t*2*r1*1e9,color="blue", linewidth = 1.0, linestyle ="-")
    plt.subplot(313)   
    plt.grid(True)
